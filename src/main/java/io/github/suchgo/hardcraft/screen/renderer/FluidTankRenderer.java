package io.github.suchgo.hardcraft.screen.renderer;
...
public class FluidTankRenderer {
    ...
    private static void drawTextureWithMasking(Matrix4f matrix, float xCord, float yCord, TextureAtlasSprite textureSprite, long maskTop, long maskRight, float zLevel) {
        ...
        bufferBuilder.vertex(matrix, xCord, yCord + 16, zLevel).uv(uMin, vMax).endVertex();
        bufferBuilder.vertex(matrix, xCord + 16 - maskRight, yCord + 16, zLevel).uv(uMax, vMax).endVertex();
        bufferBuilder.vertex(matrix, xCord + 16 - maskRight, yCord + maskTop, zLevel).uv(uMax, vMin).endVertex();
        bufferBuilder.vertex(matrix, xCord, yCord + maskTop, zLevel).uv(uMin, vMin).endVertex();
        tessellator.end();
    }
    ...
    public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
        ...
        try {
            ...
            if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
                MutableComponent amountString = Component.translatable("tooltip.hardcraft.liquid.amount.with.capacity", nf.format(milliBuckets), nf.format(capacity));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
                MutableComponent amountString = Component.translatable("tooltip.hardcraft.liquid.amount", nf.format(milliBuckets));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            }
        } catch (RuntimeException e) {
            HardCraft.LOGGER.error("Failed to get tooltip for fluid: " + e);
        }
